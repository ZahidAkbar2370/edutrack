...
<div class="d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4">
    ...
    <div class="d-flex flex-wrap gap-2">
        ...
        <a href="{{ url('attendance/export-to-csv/' . $attendanceCode) }}" class="btn btn-outline-success" data-confirm-action
        ...
        <a href="{{ url('attendance/edit/' . $attendanceCode) }}" class="btn btn-outline-primary">
            <i class="bi bi-pencil-square me-1"></i> Edit Attendance
        </a>

        <a href="{{ url('attendance/report-attendance/' . $attendanceCode) }}" class="btn btn-outline-danger" data-confirm-action
                ...
                <tbody>
                    ...
                    <tr class="attendance-row" data-student-id="{{ $attendance->student_id }}">
                        ...
                        <td>
                            ...
                            @elseif($attendance->whatsapp_status == 'processing')
                        ...
                        <td>{{ $attendance->created_at ? \Illuminate\Support\Carbon::parse($attendance->created_at)->format('d M Y h:i A') : 'N/A' }}</td>
                        <td>{{ $attendance->updated_at ? \Illuminate\Support\Carbon::parse($attendance->updated_at)->format('d M Y h:i A') : 'N/A' }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
